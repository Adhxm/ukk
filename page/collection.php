        ...
        <div class="row">
            ...
            if (isset($_SESSION['userID'])) {
                ...
                if (isset($_POST['bookmark'])) {
                    ...
                    if ($result->num_rows === 0) {
                        // Post is not bookmarked yet, add it to the collection
                        $insert_bookmark_sql = "INSERT INTO collection (userID, postID) VALUES (?, ?)";
                        $stmt = $conn->prepare($insert_bookmark_sql);
                        $stmt->bind_param("ii", $userID, $postID);
                        $stmt->execute();
                    } else {
                        // Post is already bookmarked, remove it from the collection
                ...
                // Fetch posts that are in the user's collection
                $fetch_posts_sql = "SELECT p.postID, p.caption, p.likes, p.created, p.description, p.img, u.email
                                    FROM post p
                                    LEFT JOIN likes l ON p.postID = l.postID
                                    JOIN collection c ON p.postID = c.postID
                                    JOIN user u ON p.userID = u.userID
                                    WHERE c.userID = ?
                                    GROUP BY p.postID
                                    ORDER BY p.postID DESC";
                $stmt = $conn->prepare($fetch_posts_sql);
            ...
            ?>
                        ...
                        <!-- tampilan content aseli penutup -->
            <?php
                    }
                } else {
                    echo "<p>No posts in your collection.</p>";
                }

                $stmt->close();
            } else {
                echo "<p>Please log in to view posts.</p>";
            }
            ?>
        </div>
    </div>
